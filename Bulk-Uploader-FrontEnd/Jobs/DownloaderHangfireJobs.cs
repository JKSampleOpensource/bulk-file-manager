...
public class DownloaderHangfireJobs
{
    ...
    public async Task StartBulkDownload(int bulkJobId)
    {
        ...
        else
        {
            ...
            await _dataContext.SaveChangesAsync();
            var folder = await UtilityFlows.GetRootFolder(job.HubId, job.ProjectId, job.ApsFolderId);
    ...
    public async Task ProcessFileDownload(int fileId)
    {
        try
        {
            ...
            var downloadUrl = await ForgeHelpers.GetDownloadUrl(token, bucketKey, objectName);
